...
import type { Werkzeuguebersicht, CreateWerkzeuguebersicht } from '@/types/app';
...
export class LivingAppsService {
  // --- WERKZEUGUEBERSICHT ---
  static async getWerkzeuguebersicht(): Promise<Werkzeuguebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGUEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Werkzeuguebersicht[];
    return enrichLookupFields(records, 'werkzeuguebersicht');
  }
  static async getWerkzeuguebersichtEntry(id: string): Promise<Werkzeuguebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGUEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Werkzeuguebersicht;
    return enrichLookupFields([record], 'werkzeuguebersicht')[0];
  }
  static async createWerkzeuguebersichtEntry(fields: CreateWerkzeuguebersicht) {
    return callApi('POST', `/apps/${APP_IDS.WERKZEUGUEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'werkzeuguebersicht') });
  }
  static async updateWerkzeuguebersichtEntry(id: string, fields: Partial<CreateWerkzeuguebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.WERKZEUGUEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'werkzeuguebersicht') });
  }
  static async deleteWerkzeuguebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WERKZEUGUEBERSICHT}/records/${id}`);
  }
