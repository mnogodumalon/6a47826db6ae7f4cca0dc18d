...
import type { Werkzeuguebersicht, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Werkzeuguebersicht';
...
interface WerkzeuguebersichtDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Werkzeuguebersicht['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Werkzeuguebersicht['fields'], 'kategorie' | 'zustand'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['werkzeuguebersicht']?.['kategorie'] ?? [],
  zustand: LOOKUP_OPTIONS['werkzeuguebersicht']?.['zustand'] ?? [],
};
...
export function WerkzeuguebersichtDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: WerkzeuguebersichtDialogProps) {
  const [fields, setFields] = useState<Partial<Werkzeuguebersicht['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['name', 'kategorie', 'zustand'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Werkzeuguebersicht['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'werkzeuguebersicht');
      await onSubmit(clean as Werkzeuguebersicht['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Name\n  "kategorie": LookupValue | null, // Kategorie (select one key: "handwerkzeug" | "elektrowerkzeug" | "messwerkzeug" | "schneidwerkzeug" | "spanwerkzeug" | "montagewerkzeug" | "sonstiges") mapping: handwerkzeug=Handwerkzeug, elektrowerkzeug=Elektrowerkzeug, messwerkzeug=Messwerkzeug, schneidwerkzeug=Schneidwerkzeug, spanwerkzeug=Spanwerkzeug, montagewerkzeug=Montagewerkzeug, sonstiges=Sonstiges\n  "hersteller": string | null, // Hersteller\n  "lagerort": string | null, // Lagerort\n  "zustand": LookupValue | null, // Zustand (select one key: "neu" | "gut" | "gebraucht" | "defekt" | "in_reparatur") mapping: neu=Neu, gut=Gut, gebraucht=Gebraucht, defekt=Defekt, in_reparatur=In Reparatur\n  "kaufpreis": number | null, // Kaufpreis (€)\n  "kaufdatum": string | null, // YYYY-MM-DD\n  "seriennummer": string | null, // Seriennummer\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Werkzeuguebersicht['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Werkzeugübersicht bearbeiten' : 'Werkzeugübersicht hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="kategorie" className="space-y-1.5">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="messwerkzeug">Messwerkzeug</SelectItem>
            <SelectItem value="schneidwerkzeug">Schneidwerkzeug</SelectItem>
            <SelectItem value="spanwerkzeug">Spanwerkzeug</SelectItem>
            <SelectItem value="montagewerkzeug">Montagewerkzeug</SelectItem>
    ...
    'hersteller': (
      <div key="hersteller" className="space-y-1.5">
        ...
        <Input
          id="hersteller"
          placeholder="z. B. Makita, DeWalt, Bosch"
    ...
    'lagerort': (
      <div key="lagerort" className="space-y-1.5">
        ...
        <Input
          id="lagerort"
          placeholder="z. B. Regal A3, Werkbank West"
      ...
      <div key="zustand" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand) === 'in_reparatur'}
            onClick={() => setFields(f => ({ ...f, zustand: (lookupKey(f.zustand) === 'in_reparatur' ? undefined : 'in_reparatur') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand) === 'in_reparatur'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'kaufpreis': (
      <div key="kaufpreis" className="space-y-1.5">
        <Label htmlFor="kaufpreis">Kaufpreis (€)</Label>
        <Input
          id="kaufpreis"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'kaufpreis')}
          placeholder="z. B. 89,99"
          value={fields.kaufpreis !== undefined ? fields.kaufpreis : (computedValues['kaufpreis'] ?? '')}
          onChange={e => setFields(f => ({ ...f, kaufpreis: clampNumberValue(formEnhancements, 'kaufpreis', e.target.value) }))}
        />
      </div>
    ),
    'kaufdatum': (
    ...
    'seriennummer': (
      <div key="seriennummer" className="space-y-1.5">
        ...
        <Input
          id="seriennummer"
          placeholder="z. B. MAK-2024-00142"
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, Wartung, Schäden, Zubehör..."
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name", "kategorie": "Kategorie", "hersteller": "Hersteller", "lagerort": "Lagerort", "zustand": "Zustand", "kaufpreis": "Kaufpreis (€)", "kaufdatum": "Kaufdatum", "seriennummer": "Seriennummer", "bemerkungen": "Bemerkungen", "bild": "Bild"};
  const CURRENCY_KEYS = new Set<string>(["kaufpreis"]);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WERKZEUGUEBERSICHT} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
