...
export default function WerkzeuguebersichtPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Werkzeuguebersicht[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Werkzeuguebersicht | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getWerkzeuguebersicht());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Werkzeuguebersicht['fields']) {
    await LivingAppsService.createWerkzeuguebersichtEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Werkzeuguebersicht['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateWerkzeuguebersichtEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWerkzeuguebersichtEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Werkzeugübersicht im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Werkzeugübersicht suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/werkzeuguebersicht/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Werkzeugübersicht. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Werkzeugübersicht löschen"
