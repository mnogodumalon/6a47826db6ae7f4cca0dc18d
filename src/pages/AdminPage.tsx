...
import { WerkzeuguebersichtViewDialog } from '@/components/dialogs/WerkzeuguebersichtViewDialog';
...
const WERKZEUGUEBERSICHT_FIELDS = [
  { key: 'name', label: 'Name', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'handwerkzeug', label: 'Handwerkzeug' }, { key: 'elektrowerkzeug', label: 'Elektrowerkzeug' }, { key: 'messwerkzeug', label: 'Messwerkzeug' }, { key: 'schneidwerkzeug', label: 'Schneidwerkzeug' }, { key: 'spanwerkzeug', label: 'Spanwerkzeug' }, { key: 'montagewerkzeug', label: 'Montagewerkzeug' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'hersteller', label: 'Hersteller', type: 'string/text' },
  { key: 'lagerort', label: 'Lagerort', type: 'string/text' },
  { key: 'zustand', label: 'Zustand', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'gut', label: 'Gut' }, { key: 'gebraucht', label: 'Gebraucht' }, { key: 'defekt', label: 'Defekt' }, { key: 'in_reparatur', label: 'In Reparatur' }] },
  ...
  { key: 'bild', label: 'Bild', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'werkzeuguebersicht', label: 'Werkzeugübersicht', pascal: 'Werkzeuguebersicht' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('werkzeuguebersicht');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'werkzeuguebersicht': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'werkzeuguebersicht': return (data as any).werkzeuguebersicht as Werkzeuguebersicht[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'werkzeuguebersicht': return WERKZEUGUEBERSICHT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'werkzeuguebersicht': return {
        create: (fields: any) => LivingAppsService.createWerkzeuguebersichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWerkzeuguebersichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWerkzeuguebersichtEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'werkzeuguebersicht' || dialogState?.entity === 'werkzeuguebersicht') && (
        <WerkzeuguebersichtDialog
          open={createEntity === 'werkzeuguebersicht' || dialogState?.entity === 'werkzeuguebersicht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'werkzeuguebersicht' ? handleUpdate : (fields: any) => handleCreate('werkzeuguebersicht', fields)}
          defaultValues={dialogState?.entity === 'werkzeuguebersicht' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Werkzeuguebersicht']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Werkzeuguebersicht']}
        />
      )}
      {viewState?.entity === 'werkzeuguebersicht' && (
        <WerkzeuguebersichtViewDialog
          open={viewState?.entity === 'werkzeuguebersicht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'werkzeuguebersicht', record: r }); }}
        />
      )}
